...
def test_decompress() -> None:
    ...
    try:
        ...
        assert restored_csv_path.suffix == '.restored_csv' # Original suffix is preserved by target_restore_path
    ...
    with tempfile.NamedTemporaryFile(suffix='.parquet', delete=False) as tmp_parquet_pq:
        compressed_parquet_path_pq = Path(tmp_parquet_pq.name) # This will be a copy of original_pq_path
    ...
    try:
        ...
        assert restored_pq_path.suffix == '.restored_pq' # Original suffix is preserved by target_restore_path
